...
public class RemoveCommandExecutor implements CommandExecutor
{
	public CommandResult execute(CommandSource src, CommandContext ctx) throws CommandException
	{
		...
		if (src instanceof Player)
		{
			...
			CommandSigns.commandSignModifiers.add(new CommandSignModifier(commandNumber, player.getUniqueId()));
			player.sendMessage(Text.of(TextColors.DARK_RED, "[CommandSigns]: ", TextColors.GOLD, "Right click a CommandSign!"));
		}
		else
		{
			src.sendMessage(Text.of(TextColors.DARK_RED, "Error! ", TextColors.RED, "Must be an in-game player to use /removecommand!"));
		}
